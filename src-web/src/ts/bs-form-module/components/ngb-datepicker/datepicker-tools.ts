...
export function isDateSelectable(months: MonthViewModel[], date: NgbDate) {
  ...
  const tempMonth = months.filter(curMonth => curMonth.year === date.year && curMonth.number === date.month)
  let month = null
  if (tempMonth.length > 0) { month = tempMonth[0]}

  if (month) {


    month.weeks.forEach(week => {
        const tempDay = week.days.filter(dayItem => date.equals(dayItem.date))
        let day = null
        if (tempDay.length > 0) { day = tempDay[0]}
...
export function buildMonths(
  ...
  for (let i = 0; i < displayMonths; i++) {
    ...
    let index = -1
      months.forEach ( (month, indexItem ) => {
          if (month.firstDate.equals(newDate) && index === -1) {
              index = indexItem
          }
      })
