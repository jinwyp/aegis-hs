...
create table hs_cang_huikuan (
   ...
   huikuanPaper tincangt                           comment '是否收到票据, 如果回款方式是银行承兑, 此字段有效',
   huikuanPaperDate datetime                      comment '收到票据原件日期, 如果收到票据',
   huikuanDiscount tincangt                        comment '是否贴息, 如果回款方式是银行承兑, 此字段有效',
...
-- 仓押订单 - 下游结算-发运map
create table hs_cang_settle_downstream_map (
   id bigint(20)             not null auto_increment,
