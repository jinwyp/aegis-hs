...
create table hs_same_order (
  ...
  terminalClientId  bigint(20)              comment '终端客户id',
...
create table hs_same_order_config (
  ...
  maxPrepayRate decimal(10, 4)        not null comment '最高预付款比例',
  unInvoicedRate decimal(10, 4)       not null comment '未开票款付款比例',
  contractBaseInterest decimal(10, 4) not null comment '合同基准利率',
...
create table hs_same_fee (
  ...
  otherPartyId bigint(20)            not null comment '对方单位',
  quantity decimal(10, 2)    null  comment '数量 单位吨',
...
create table hs_same_huikuan (
  ...
  huikuanBankDiscountRate decimal(10, 4)             comment '如果回款方式是银行承兑, 贴息率',
  ...
  huikuanBusinessDiscountRate decimal(10, 4)         comment '如果回款方式是商业承兑, 贴息率',
...
create table hs_same_log (
  ...
  memo TEXT       not null comment '修改日志',
  deleted tinyint(1)          not null default 0 comment '逻辑删除',
  tsc timestamp           not null default current_timestamp,
  tsu timestamp not null default current_timestamp,
