...
//    @After("execution(* com.yimei.hs.same.service..*.create(..))")
    public void create(JoinPoint joinPoint) {
        ...
        if (arg instanceof Order) {
            ...
            logService.create(settleTraffic, EntityType.cangSettleTrafficInsert);
        } else if (arg instanceof Fee) {
            Fee fee = (Fee) arg;
            Order order = orderMapper.selectByPrimaryKey(fee.getOrderId());
            logService.create(fee, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFeeInsert : EntityType.cangFeeInsert));
            ...
            logService.create(orderParty, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingPartyInsert : EntityType.cangPartyInsert));
        } else if (arg instanceof Invoice) {
            Invoice invoice = (Invoice) arg;
            Order order = orderMapper.selectByPrimaryKey(invoice.getOrderId());
            logService.create(invoice, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingInvoiceInsert : EntityType.cangInvoiceInsert));
...
//            logService.create(om, orderMapper, logService, invoiceDetail,(order.getBusinessType().equals(BusinessType.ying)?EntityType.yingConfigInsert:EntityType.cangOrderInsert));
        } else if (arg instanceof Huikuan) {

            Huikuan huikuan = (Huikuan) arg;
            Order order = orderMapper.selectByPrimaryKey(huikuan.getOrderId());
            logService.create(huikuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuikuanInsert : EntityType.cangHuankuanInsert));
            ...
            logService.create(huankuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuankuanInsert : EntityType.cangHuankuanInsert));

        } else if (arg instanceof Jiekuan) {

            Jiekuan jiekuan = (Jiekuan) arg;
            Order order = orderMapper.selectByPrimaryKey(jiekuan.getOrderId());
            logService.create(jiekuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingJiekuanInsert : EntityType.cangJiekuanInsert));

        } else if (arg instanceof Fukuan) {

            Fukuan fukuan = (Fukuan) arg;
            Order order = orderMapper.selectByPrimaryKey(fukuan.getOrderId());
            logService.create(fukuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFukuanInsert : EntityType.cangFukuanInsert));
            ...
            logService.create(settleSeller, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleUpstreamInsert : EntityType.cangSettleUpstreamInsert));
            ...
            logService.create(settleBuyer, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleDownInsert : EntityType.cangSettleDownInsert));
...
//    @After("execution(* com.yimei.hs.same.service..*.update*(..))")
    public void update(JoinPoint joinPoint) {
        ...
        if (arg instanceof Order) {
            ...
            logService.create(settleTraffic, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleTrafficUpdate: EntityType.cangSettleTrafficUpdate));
        } else if (arg instanceof Fee) {
            Fee fee = (Fee) arg;
            Order order = orderMapper.selectByPrimaryKey(fee.getOrderId());
            logService.create(fee, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFeeUpdate: EntityType.cangFeeUpdate));
        } else if (arg instanceof OrderConfig) {
            OrderConfig orderConfig = (OrderConfig) arg;
            Order order = orderMapper.selectByPrimaryKey(orderConfig.getOrderId());
            logService.createforConfig(
            ...
            OrderParty orderParty = (OrderParty) arg;
            Order order = orderMapper.selectByPrimaryKey(orderParty.getOrderId());
            logService.create(orderParty, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingPartyUpdate : EntityType.cangPartyUpdate));
        } else if (arg instanceof Invoice) {
            Invoice invoice = (Invoice) arg;
            Order order = orderMapper.selectByPrimaryKey(invoice.getOrderId());
            logService.create(invoice, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingInvoiceUpdate : EntityType.cangInvoiceUpdate));
        } else if (arg instanceof InvoiceDetail) {

            InvoiceDetail invoiceDetail = (InvoiceDetail) arg;
//            Order order = orderMapper.selectByPrimaryKey(invoiceDetail.getOrderId());
//            logService.create(om, orderMapper, logService, invoiceDetail,(order.getBusinessType().equals(BusinessType.ying)?EntityType.yingConfigUpdate:EntityType.cangOrderUpdate));
        } else if (arg instanceof Huikuan) {

            Huikuan huikuan = (Huikuan) arg;
            Order order = orderMapper.selectByPrimaryKey(huikuan.getOrderId());
            logService.create(huikuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuikuanUpdate : EntityType.cangHuankuanUpdate));

        } else if (arg instanceof Huankuan) {

            Huankuan huankuan = (Huankuan) arg;
            Order order = orderMapper.selectByPrimaryKey(huankuan.getOrderId());
            logService.create(huankuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuankuanUpdate : EntityType.cangHuankuanUpdate));

        } else if (arg instanceof Jiekuan) {

            Jiekuan jiekuan = (Jiekuan) arg;
            Order order = orderMapper.selectByPrimaryKey(jiekuan.getOrderId());
            logService.create(jiekuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingJiekuanUpdate : EntityType.cangJiekuanUpdate));

        } else if (arg instanceof Fukuan) {

            Fukuan fukuan = (Fukuan) arg;
            Order order = orderMapper.selectByPrimaryKey(fukuan.getOrderId());
            logService.create(fukuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFukuanUpdate : EntityType.cangFukuanUpdate));

        } else if (arg instanceof SettleSeller) {

            SettleSeller settleSeller = (SettleSeller) arg;
            Order order = orderMapper.selectByPrimaryKey(settleSeller.getOrderId());
            logService.create(settleSeller, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleUpstreamUpdate : EntityType.cangSettleUpstreamUpdate));

        } else if (arg instanceof SettleBuyer) {
            SettleBuyer settleBuyer = (SettleBuyer) arg;
            Order order = orderMapper.selectByPrimaryKey(settleBuyer.getOrderId());
            logService.create(settleBuyer, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleDownUpdate : EntityType.cangSettleDownUpdate));
        }
    }
...
//    @After("execution(* com.yimei.hs.same.service..*.delete(..))")
    public void delete(JoinPoint joinPoint) {
        ...
        if (fileName.equals(OrderService.class.getName())) {
            ...
            Order order = orderMapper.selectByPrimaryKey(settleTraffic.getOrderId());
            logService.create(settleTraffic, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleTrafficDel: EntityType.cangSettleTrafficDel));
        } else if ( fileName.equals(FeeService.class.getName())) {
            Fee fee = feeMapper.selectByPrimaryKeyDeleted(id) ;
            Order order = orderMapper.selectByPrimaryKey(fee.getOrderId());
            logService.create(fee, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFeeDel: EntityType.cangFeeDel));
            ...
            Order order = orderMapper.selectByPrimaryKey(orderParty.getOrderId());
            logService.create(orderParty, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingPartyDel: EntityType.cangPartyDel));
        } else if (fileName.equals(InvoiceService.class.getName())) {
            Invoice invoice = invoiceMapper.selectByPrimaryKeyDeleted(id);
            Order order = orderMapper.selectByPrimaryKey(invoice.getOrderId());
            logService.create(invoice, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingInvoiceDel: EntityType.cangInvoiceDel));
        } else if (fileName.equals(HuikuanService.class.getName())) {

            Huikuan huikuan = huikuanMapper.selectByPrimaryKeyDeleted(id);
            Order order = orderMapper.selectByPrimaryKey(huikuan.getOrderId());
            logService.create(huikuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuikuanDel: EntityType.cangHuankuanDel));

        } else if (fileName.equals(HuankuanService.class.getName())) {

            Huankuan huankuan = huankuanMapper.selectByPrimaryKeyDeleted(id);
            Order order = orderMapper.selectByPrimaryKey(huankuan.getOrderId());
            logService.create(huankuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuankuanDel: EntityType.cangHuankuanDel));

        } else if (fileName.equals(JiekuanService.class.getName())) {

            Jiekuan jiekuan = jiekuanMapper.selectByPrimaryKeyDeleted(id);
            Order order = orderMapper.selectByPrimaryKey(jiekuan.getOrderId());
            logService.create(jiekuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingJiekuanDel: EntityType.cangJiekuanDel));

        } else if (fileName.equals(FukuanService.class.getName())) {

            Fukuan fukuan = fukuanMapper.selectByPrimaryKeyDeleted(id);
            Order order = orderMapper.selectByPrimaryKey(fukuan.getOrderId());
            logService.create(fukuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFukuanDel: EntityType.cangFukuanDel));
            ...
            Order order = orderMapper.selectByPrimaryKey(settleSeller.getOrderId());
            logService.create(settleSeller, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleUpstreamDel: EntityType.cangSettleUpstreamDel));
            ...
            Order order = orderMapper.selectByPrimaryKey(settleBuyer.getOrderId());
            logService.create(settleBuyer, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleDownDel: EntityType.cangSettleDownDel));
        }
    }
}
