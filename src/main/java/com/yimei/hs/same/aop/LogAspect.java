...
//            Logutil.create(om, orderMapper, logService, invoiceDetail,(order.getBusinessType().equals(BusinessType.ying)?EntityType.yingConfigInsert:EntityType.cangOrderInsert));
    ...
    public void update(JoinPoint joinPoint) {
        ...
        if (arg instanceof Order) {
            ...
            Logutil.create(om, orderMapper, logService, settleTraffic, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleTrafficUpdate: EntityType.cangSettleTrafficUpdate));
        } else if (arg instanceof Fee) {
            Fee fee = (Fee) arg;
            Order order = orderMapper.selectByPrimaryKey(fee.getOrderId());
            Logutil.create(om, orderMapper, logService, fee, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFeeUpdate: EntityType.cangFeeUpdate));
        } else if (arg instanceof OrderConfig) {
            OrderConfig orderConfig = (OrderConfig) arg;
            Order order = orderMapper.selectByPrimaryKey(orderConfig.getOrderId());
            Logutil.createforConfig(om,
            ...
            OrderParty orderParty = (OrderParty) arg;
            Order order = orderMapper.selectByPrimaryKey(orderParty.getOrderId());
            Logutil.create(om, orderMapper, logService, orderParty, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingPartyUpdate : EntityType.cangPartyUpdate));
        } else if (arg instanceof Invoice) {
            Invoice invoice = (Invoice) arg;
            Order order = orderMapper.selectByPrimaryKey(invoice.getOrderId());
            Logutil.create(om, orderMapper, logService, invoice, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingInvoiceUpdate : EntityType.cangInvoiceUpdate));
        } else if (arg instanceof InvoiceDetail) {

            InvoiceDetail invoiceDetail = (InvoiceDetail) arg;
//            Order order = orderMapper.selectByPrimaryKey(invoiceDetail.getOrderId());
//            Logutil.create(om, orderMapper, logService, invoiceDetail,(order.getBusinessType().equals(BusinessType.ying)?EntityType.yingConfigUpdate:EntityType.cangOrderUpdate));
        } else if (arg instanceof Huikuan) {

            Huikuan huikuan = (Huikuan) arg;
            Order order = orderMapper.selectByPrimaryKey(huikuan.getOrderId());
            Logutil.create(om, orderMapper, logService, huikuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuikuanUpdate : EntityType.cangHuankuanUpdate));

        } else if (arg instanceof Huankuan) {

            Huankuan huankuan = (Huankuan) arg;
            Order order = orderMapper.selectByPrimaryKey(huankuan.getOrderId());
            Logutil.create(om, orderMapper, logService, huankuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingHuankuanUpdate : EntityType.cangHuankuanUpdate));

        } else if (arg instanceof Jiekuan) {

            Jiekuan jiekuan = (Jiekuan) arg;
            Order order = orderMapper.selectByPrimaryKey(jiekuan.getOrderId());
            Logutil.create(om, orderMapper, logService, jiekuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingJiekuanUpdate : EntityType.cangJiekuanUpdate));

        } else if (arg instanceof Fukuan) {

            Fukuan fukuan = (Fukuan) arg;
            Order order = orderMapper.selectByPrimaryKey(fukuan.getOrderId());
            Logutil.create(om, orderMapper, logService, fukuan, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingFukuanUpdate : EntityType.cangFukuanUpdate));

        } else if (arg instanceof SettleSeller) {

            SettleSeller settleSeller = (SettleSeller) arg;
            Order order = orderMapper.selectByPrimaryKey(settleSeller.getOrderId());
            Logutil.create(om, orderMapper, logService, settleSeller, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleUpstreamUpdate : EntityType.cangSettleUpstreamUpdate));

        } else if (arg instanceof SettleBuyer) {
            SettleBuyer settleBuyer = (SettleBuyer) arg;
            Order order = orderMapper.selectByPrimaryKey(settleBuyer.getOrderId());
            Logutil.create(om, orderMapper, logService, settleBuyer, (order.getBusinessType().equals(BusinessType.ying) ? EntityType.yingSettleDownUpdate : EntityType.cangSettleDownUpdate));
        }
    }
