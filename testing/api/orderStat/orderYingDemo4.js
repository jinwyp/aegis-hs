    ...
    it(`统计验证 - 获取核算月统计信息 GET: /api/business/ying/${orderId}/analysis/${unitId}`, function (done) {
        server.get(`/api/business/ying/${orderId}/analysis/${unitId}`)
            ...
            .end(function(err, res) {
                ...
                expect(res.body.data.tradeCompanyAddMoney, '毛利贸易公司加价数据不对').to.equal(20546.00)
                expect(res.body.data.vat, '应交增值税数据不对').to.equal(600.04)
                expect(res.body.data.withoutTaxIncome, '不含税收入数据不对').to.equal(1229247.86)
                expect(res.body.data.withoutTaxCost, '不含税成本数据不对').to.equal(1225718.23)
                expect(res.body.data.additionalTax, '税金及附加数据不对').to.equal(72.00)
                expect(res.body.data.stampDuty, '印花税数据不对').to.equal(861.69)
