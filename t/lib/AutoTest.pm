...
sub get {
    ...
    my $cmd;
    if ($args->{jwt}) {
        $cmd = "curl -s -H \"Authorization: Bearer $self->{jwt}\" --cookie-jar target/cookie --cookie \"$self->{c}\" -XGET \"http://$self->{host}$url\"";
    } else {
        $cmd = "curl -s --cookie-jar target/cookie --cookie \"$self->{c}\" -XGET \"http://$self->{host}$url\"";
    }
...
sub cookie {
    my $self = shift;
    my $cookie = `cat target/cookie | grep clientId | awk '{ print \$7;}'`;
    chomp $cookie;
    $self->{c} = "clientId=$cookie";
}
